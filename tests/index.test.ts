...
import { APIPromise } from 'Met-Museum-Demo/core/api-promise';

import util from 'node:util';
import MetMuseumDemo from 'Met-Museum-Demo';
import { APIUserAbortError } from 'Met-Museum-Demo';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['MET_MUSEUM_DEMO_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: MetMuseumDemo) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new MetMuseumDemo({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new MetMuseumDemo({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new MetMuseumDemo({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['MET_MUSEUM_DEMO_LOG'] = 'debug';
      const client = new MetMuseumDemo({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['MET_MUSEUM_DEMO_LOG'] = 'not a log level';
      const client = new MetMuseumDemo({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'MET_MUSEUM_DEMO_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['MET_MUSEUM_DEMO_LOG'] = 'debug';
      const client = new MetMuseumDemo({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['MET_MUSEUM_DEMO_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new MetMuseumDemo({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MET_MUSEUM_DEMO_BASE_URL'] = 'https://example.com/from_env';
      const client = new MetMuseumDemo({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['MET_MUSEUM_DEMO_BASE_URL'] = ''; // empty
      const client = new MetMuseumDemo({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://collectionapi.metmuseum.org/public/collection/v1');
    });

    test('blank env variable', () => {
      process.env['MET_MUSEUM_DEMO_BASE_URL'] = '  '; // blank
      const client = new MetMuseumDemo({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://collectionapi.metmuseum.org/public/collection/v1');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new MetMuseumDemo({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new MetMuseumDemo({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new MetMuseumDemo({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new MetMuseumDemo({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new MetMuseumDemo({ apiKey: 'My API Key', fetch: testFetch });
